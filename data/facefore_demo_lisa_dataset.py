...
class FaceForeDemoLisaDataset(BaseDataset):
    @staticmethod
    ...
    def initialize(self,opt):
        ...
        # read in data
        self.tgt_lmarks = np.load(self.tgt_lmarks_path)[:170]
        self.ref_lmarks = np.load(self.ref_lmarks_path)
        self.ref_image = cv2.imread(self.ref_video_path)
        if self.opt.warp_ani:
            self.tgt_ani_video = self.read_videos(self.tgt_ani_path)[30:200]
            self.ref_front = np.load(self.ref_front_path)
            self.ref_ani_id = self.opt.ref_ani_id
        if self.opt.warp_ani or self.ref_search:
            self.tgt_rt = np.load(self.tgt_rt_path)[30:200]
        ...
        self.ref_video, self.ref_lmarks = self.define_inference([self.ref_image], [self.ref_lmarks])


    def __len__(self):
        return self.tgt_lmarks.shape[0]
    ...
    def __getitem__(self, index):
        # get target
        target_id = [index]
        tgt_lmarks = [self.get_keypoints(self.tgt_lmarks[target_id[0]], self.transform_L, self.output_shape)]
        # get animation
        if self.opt.warp_ani:
        ...
        warping_refs = self.ref_video
        warping_ref_lmarks = self.ref_lmarks
        ...
        input_dic = {'index':target_id, 'tgt_label': tgt_lmarks, \
    ...
    def define_inference(self, real_video, lmarks):
        # get face image
        ref_images, ref_lmarks = self.prepare_datas(real_video, lmarks, [0])
