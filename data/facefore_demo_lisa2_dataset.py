...
class FaceForeDemoLisa2Dataset(BaseDataset):
    @staticmethod
    ...
    def initialize(self,opt):
        ...
        if len(self.ref_lmarks.shape) == 2:
            self.ref_lmarks = np.expand_dims(self.ref_lmarks, axis=0)
    ...
    def name(self):
        return 'FaceForeDemoLisa2Dataset'

    def __getitem__(self, index):
        # get target
        target_id = [index]
        tgt_images, tgt_lmarks = self.prepare_datas(None, self.tgt_lmarks, target_id)
        ...
        target_img_path = [os.path.join(self.tgt_lmarks_path[:-4] , '%05d.png'%t_id) for t_id in target_id]
        ...
        input_dic = {'path': self.tgt_lmarks_path, 'v_id' : target_img_path, 'index':target_id, 'tgt_label': tgt_lmarks, \
    ...
    def get_template(self, lmark, transform_T):
        # crop
        template = get_roi(lmark, mask_eyes=False)
